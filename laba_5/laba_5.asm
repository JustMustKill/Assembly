...
main:
	...
	call convertInNumber
							;
	call mainProc           ; Главная процедура, в которой содержится весь алгоритм обработки файла
	cmp ax, 0               ; 
	jne endMain				;  ==//==
                            ;
...
loopParseNumber:	
	...
	je isStoppedNumber          ;         Если этот символ равен            ;
	cmp al, tabulation          ;   пробелу, табуляции, 0Ah, 0Dh или \0
	je isStoppedNumber          ;     Значит мы дошли до  конца слова
	cmp al, returnSymbol        ;
	je isStoppedNumber          ;                                        ;
	cmp al, endl                ;
	je isStoppedNumber          ;
	...
	mov es:[di], al             ; Если данный символ является цифрой, добавляем его в результирующую строку
                                ;
	inc di                      ; Увеличиваем di,si т.е. переходим на следующий символ
	inc si  					;
								;
loop loopParseNumber            ; Пока не превышена максимальная длина слова, парсим
isStoppedNumber:  
								;
	mov al, endl                ;
...
clearSymbol:	
	cmp clearMode, 1
	jne skipSymbol
	mov byte ptr ds:[si], spaceSymbol
	jmp skipSymbol
...
findNewLine MACRO
	...
	mov al, ds:[si]                     ; Записываем в al символ, подлежащий записи
	                                    ...
	                                    ;
	cmp al, newLineSymbol               ;
	je isNewLineSymbo1                  ;         Если этот символ равен
                                        ;
										;   пробелу, табуляции, 0Ah, 0Dh или \0
										;
	...
	je isNewLineSymbo1                  ;
                                        ;
                                        ;                                       ;
	jmp isNotDe1im                      ; Иначе прыгаем в isNotDelim
                                        ;
isNewLineSymbo1:                        ;
...
;	je notNewLine                       ; Если ax == 0 прыгаев в notNewLine
                                        ; 
                                        ; Если переходим на новую строку, то необходимо обнулить индекс текущего слова                   
...
skipWrite:                      ;
	...
	je endOfProcessing          ;
                                ;
	lea si, buffer              ; Загружаем в si смещение буффера, в который считывали данные из файла
								;
